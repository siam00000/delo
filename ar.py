...
## Don't buy and sell pussy...
## Respect me by not changing the author's name...
## And don't forget to subscribe to my channel mmk...
## For those who want to donate to funds -> 0812118***6
## MUST FUND AJG:V


## THANK YOU FOR THOSE WHO HAVE SUBSCRIBED TO ME / DONATION MKSH...
## Import Module Kntl ..
...
class menu:
	...
	def main(self):
		...
		print(' %s║'%(N))
print(' %s╠═[%s•%s] Access your licensee : %s'%(N,O,N,akss))
print(' %s╠═[%s•%s] Your current IP address : %s'%(N,O,N,IP))
print(' %s╠═[%s•%s] You are logged in : %s'%(N,O,N,time))
print(' %s║'%(N))
...
print(' %s╠═[%s4%s] crack of likes post [ON]'%(N,O,N))
print(' %s╠═[%s5%s] crack from followers [ON]'%(N,O,N))
print(' %s╠═[%s6%s] check account options checkpoint [ON]'%(N,O,N))
print(' %s╠═[%s7%s] check crack ok,cp [ON]'%(N,O,N))
print(' %s╠═[%s8%s] setting User-Agent [ON]'%(N,O,N))
print(' %s╠═[%s9%s] crack email [ON]'%(N,O,N))
print(' %s╠═[%sG%s] Get data² facebook [ON]'%(N,O,N))
print(' %s╠═[%sK%s] Bug report script [ON]'%(N,O,N))
print(' %s╠═[%sA%s] Exit, delete token[ [ON]'%(N,O,N))
